...
def objective(params):        
    ...
    for i in range(3):
        #initialize
        model = RVFLRegressor(hid_num=int(params['neurons']))
        #model train
...
for h in range(4):    
    ...
    for k in range(len(espiras)):
        ...
        # load Dataset
        df = pd.read_csv('dataset_TRAIN_NYC/'+ espiras[k]+'train_NYC.csv') 
        ...
        if h==0:
         ...
         # Parameter Space
        SPACE_FNN = dict([('neurons',  hp.quniform('neurons',10,100,1))
    ...
    with open('trials/sRVFL_trials_t+'+str(h+1)+'.pkl', 'wb') as f:
