...
from skRVFL import RVFLRegressor
...
for h in range(4):    
    ...
    with open('trials/sRVFL_trials_t+'+str(h+1)+'.pkl', 'rb') as f:
    ...
    for k in range(len(espiras)):
        ...
        model = RVFLRegressor(hid_num=(
            int(np.array(params['neurons']))))
    ...
    with open('metrics/sRVFL_metrics_t+'+str(h+1)+'.pkl', 'wb') as f:
