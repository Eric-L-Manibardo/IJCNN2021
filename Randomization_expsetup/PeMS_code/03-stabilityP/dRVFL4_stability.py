...
for h in range(4):    
    ...
    for k in range(len(espiras)):
        ...
        X_train = train_scaler.fit_transform(df_train.iloc[:,1:6].values)
        # load TEST
        df_test = pd.read_csv('dataset_TEST_PeMS/'+ espiras[k]+'test_PeMS.csv') 
        #Pasamos a array target value y features
        y_test = df_test['target'].values 
        X_test = train_scaler.transform(df_test.iloc[:,1:6].values)
...
df.to_csv('stability/dRVFL4_stability.csv')
